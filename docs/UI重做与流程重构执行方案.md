...
> 状态：**Option C 已落地（`1e4772c`）；P3c 布局重构已落地（`061b974`：转轮沉底常驻 dock + 中央动画舞台 + 移除游戏内整备入口）；P4 动画 / 商店抽屉 / P5 立绘 待实施**。均待用户 F6 复验
> 最后更新：2026-08-05
...
| 编号 | 决策 | 说明 |
|---|---|---|
| **D1** | 转轮沉到底部常驻 dock，中央挖出动画舞台 | 战斗"生动"化，眼睛跟中央小人打架而非数字 |
...
```
┌──────────────────────────────────────────────────────────┐
│ ❤75/175 🛡12 🏛5🔨 💰230   4/12 (Act I)  [🔧整备][🛒商店][▶下一房] │ ← 顶栏：玩家状态 + 进度 + 间歇态三按钮
├──────────┬───────────────────────────────┬────────────────┤
│          │                               │                │
│ 玩家立绘 │        ★ 中央动画舞台 ★        │   敌人立绘      │
│ 🔮护符   │   (TextureRect / Control 区)   │   HP ▓▓▓░      │
│ 🛡增益   │   🗡️             💀           │   意图 ⚔        │
│          │  (攻击/受击/暴击⚡/核爆动画)    │   护甲 🛡       │
├──────────┴───────────────────────────────┴────────────────┤
│ [💊][💊][💎][✨]    ┃┃┃ 转轮常驻    [ ▶ SPIN ]              │ ← 底栏 dock：消耗品腰带 + 转轮 + SPIN
└──────────────────────────────────────────────────────────┘
   ▔▔▔▔▔▔▔▔ 上方一条半透明伤害日志条（薄，可卷动）▔▔▔▔▔▔▔
```
- **转轮从「中栏」搬「底栏常驻」**：即使敌人回合/结算中也不消失，玩家随时可见本手牌。
...
| 底栏 dock | `HBoxContainer` | `anchors_preset=15` + `anchor_top=0.85` | 消耗品腰带(4槽) + 转轮 + SPIN |
