...
| 解法·组合型新解法 | **必须引入新内容**（新武器 / 新护符 / 新技能 / 新消耗品等，≥1 类）形成的**组合式解法**，**数量规定为 2 个**（2026-08-10 拍板：每个 BOSS 固定设计 2 个组合型新解法，不多不少）；新内容如何组合、新增哪些品类自由发挥，但解法个数固定 2；作为该 BOSS 的特色解法/奖励导向；须符合 §8 自由度（单侧性纪律、克制动词矩阵内、元素不变量）与 T4 内容规模锚点 |
...
- 约束：单侧性纪律（不跨玩家/敌人侧共享状态）、克制动词矩阵内"基线可解"（§6：现有物品框架必须可解；**组合型新解法固定 2 个，须引入新内容（≥1 类）形成组合式解法，组合方式/品类自由**）
...
④ 验证可解（现有物品框架内基线可解？；**组合型新解法固定 2 个 = 必须引入新内容（新武器/护符/消耗品/技能等 ≥1 类）形成的组合式解法**，组合方式/品类自由，随 BOSS 引入——§6）
