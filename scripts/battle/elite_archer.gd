...
func _on_ai_attack(target: Node2D, damage: float):
	...
	_setup_arrow_visual(bullet)


func _setup_arrow_visual(bullet):
	var spr = bullet.get_node_or_null("Sprite2D")
	if spr:
		spr.texture = DamageSystem.get_circle_texture(6, Color(0.8, 0.7, 0.4), Color(0.9, 0.85, 0.6))
		spr.centered = true
