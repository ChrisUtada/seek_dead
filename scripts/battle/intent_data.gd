...
# 显示名/图标/权重/可净化/数值公式参数。默认表按幕分档（StatusSystem.ACT_INTENT_WEIGHTS + INTENT_UNLOCK_ACT 入池幕号）；
# 房间想特化 → RoomData.intents 填自己的 Array[IntentData]（空 = 行为族剖面/按幕分档默认表）。
...
@export var weight: float = 1.0        # 房间表内相对权重（RoomData.intents 使用；默认表权重见 StatusSystem.ACT_INTENT_WEIGHTS）
