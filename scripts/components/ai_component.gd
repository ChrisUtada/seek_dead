...
enum AIState { IDLE, WANDER, PATROL, ALERT, COMBAT, RETURN }
...
func set_behaviors(types: Array[int], durations: Array[float] = []):
	...
	if behaviors.is_empty():
		behaviors = [BehaviorType.CHASE_MELEE]
		behavior_durations = [-1.0]
	if behavior_durations.size() < behaviors.size():
...
func _process_combat(delta):
	...
	if _get_distance_to(_target) >= detect_range * 1.5:
		_lost_timer += delta
		if _lost_timer > 3.0:
	...
	if _behavior_timer <= 0 and behavior_durations.size() > behavior_index and behavior_durations[behavior_index] >= 0:
		_advance_behavior()

	var bt = behaviors[behavior_index] if behavior_index < behaviors.size() else BehaviorType.CHASE_MELEE
	match bt:
		BehaviorType.CHASE_MELEE:
			_tick_chase_melee(delta)
		BehaviorType.STRAFE:
			_tick_strafe(delta)
		BehaviorType.RETREAT:
			_tick_retreat(delta)
		BehaviorType.TURRET:
			_tick_turret(delta)
		BehaviorType.SUMMON:
...
func _advance_behavior():
	...
	_behavior_timer = behavior_durations[behavior_index] if behavior_index < behavior_durations.size() else -1.0
	_in_attack_phase = false


func _tick_chase_melee(delta):
	var dist = _get_distance_to(_target)
	if dist < attack_range:
		_last_move_dir = Vector2.ZERO
		_in_attack_phase = true
		if _attack_timer <= 0:
...
func _tick_strafe(delta):
	var dist = _get_distance_to(_target)
	var preferred = attack_range * 0.7
	if dist < preferred * 0.6:
		var away = get_parent().global_position.direction_to(_target.global_position) * -1
		var perp = Vector2(-away.y, away.x)
		var angle = fmod(float(get_parent().get_instance_id()) * 0.0013, TAU)
		var side = cos(angle)
		_nav.target_position = get_parent().global_position + (away * preferred * 0.5) + (perp * side * preferred * 0.8)
	elif dist > preferred * 1.4:
		_nav.target_position = _target.global_position
	else:
		var perp = Vector2(0, 1)
		var angle = fmod(float(get_parent().get_instance_id()) * 0.0013, TAU)
		var dir = Vector2(cos(angle), sin(angle))
		var orbit = _target.global_position + dir * preferred
		_nav.target_position = orbit
	if _nav.is_navigation_finished():
		_last_move_dir = Vector2.ZERO
	else:
		var next = _nav.get_next_path_position()
...
func _tick_retreat(delta):
	var dist = _get_distance_to(_target)
	var preferred = attack_range * 0.9
	if dist < preferred * 0.5:
		var away = get_parent().global_position.direction_to(_target.global_position) * -1
		_nav.target_position = get_parent().global_position + away * preferred * 1.5
...
func _tick_summon(delta):
	var dist = _get_distance_to(_target)
	var safe_dist = attack_range * 1.5
	if dist < safe_dist:
		var away = get_parent().global_position.direction_to(_target.global_position) * -1
		_nav.target_position = get_parent().global_position + away * safe_dist
		if _nav.is_navigation_finished():
			_last_move_dir = Vector2.ZERO
		else:
			var next = _nav.get_next_path_position()
...
func _tick_flee(delta):
	_last_move_dir = Vector2.ZERO
	var away = get_parent().global_position.direction_to(_target.global_position) * -1
	var flee_pos = get_parent().global_position + away * 300.0
	flee_pos = Vector2(
		clamp(flee_pos.x, _nav_bounds.position.x + 20, _nav_bounds.end.x - 20),
		clamp(flee_pos.y, _nav_bounds.position.y + 20, _nav_bounds.end.y - 20),
	)
	_nav.target_position = flee_pos
	if _nav.is_navigation_finished():
		_last_move_dir = Vector2.ZERO
	else:
		var next = _nav.get_next_path_position()
...
func is_attacking() -> bool:
	return current_state == AIState.COMBAT and _in_attack_phase
