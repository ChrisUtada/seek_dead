...
#   并因 UI 含中文（整备/武器/技能…），下限抬到 ≥9px 保证 CJK 像素字可读。
#   调整全局字号只需改这里。
...
const SUBTITLE := 11  # 主操作按钮（SPIN）
const BODY     := 11  # 正文标签（玩家/敌人 / 铁砧标题）
const MEDIUM   := 10  # 数值行（HP、确认/返回按钮）
const META     := 10  # 信息栏（房间/回合/护盾/副按钮/卡片名）
const TINY     := 9   # 弱信息（本局加成/日志/描述/图例/tooltip）
const CAPTION  := 9   # 极弱说明（副标题下一行的小字/升级花费）
