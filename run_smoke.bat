...
echo [1/8] L0 intent act-gating regression ...
...
echo [2/8] L0 boss loot theme regression ...
...
echo [3/8] L1 content and math lint ...
...
echo [4/8] L2+L3 full-run smoke and boss phase probes, about 40s ...
...
echo [5/8] T8 drop channels regression, about 60s ...
...
echo [6/8] T7 acquisition weighting regression, about 60s ...
...
echo [7/8] curriculum mini-mechanics regression ...
...
echo [8/8] charge gating micro-probe ...
"%GODOT%" --headless --fixed-fps 60 --path . res://tests/charge_rule_test.tscn > "%TEMP%\smoke_chg.log" 2>&1
if errorlevel 1 (
  set FAIL=1
  echo   FAIL -- see %TEMP%\smoke_chg.log
) else (
  echo   ok
)
